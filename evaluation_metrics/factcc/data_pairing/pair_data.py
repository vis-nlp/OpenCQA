...
def main(args):
    ...
    for path, _, filenames in os.walk(args.unpaired_data):
        for filename in filenames:
            ...
            with open(unpaired_path,encoding='utf-8') as fd:
                dataset = [json.loads(line,encoding='utf-8') for line in fd]

            for example in tqdm(dataset):
                story_path = os.path.join(args.story_files, example["filepath"])

                with open(story_path,encoding='utf-8') as fd:
            ...
            with open(paired_path, "w",encoding='utf-8') as fd:
