...
def random_selection(article):
    summ_len = len(article.split())
    overlap = 0.045
    end_token = int(summ_len - summ_len * overlap)+1
    random_start_token = random.randint(0,end_token)
    return " ".join(article.split()[random_start_token:random_start_token+summ_len - end_token])


all_annots = pd.read_csv("../data(full_summary_article)/all_annotations_extended.csv",encoding='utf8',index_col=0)



train = load_json_file("../data(full_summary_article)/train_extended.json")
test = load_json_file("../data(full_summary_article)/test_extended.json")
val = load_json_file("../data(full_summary_article)/val_extended.json")
...
for dataset,name in [(train,'train'),(test,'test'),(val,'val')]:
    ...
    save_json_file(name + "_randarticle.json",dataset)


train.update(val)
train.update(test) 
save_json_file('full_dataset_randarticle.json',train)
...
all_annots['random_article'] = randsumm_list

all_annots.to_csv("all_annotations_randarticle.csv",encoding='utf8', line_terminator='\n')
...
# for key in test:
#     randsumm_list.append(test[key][2])


# with open("test_randarticle.txt", 'w',encoding='utf8') as f:
#     for item in randsumm_list:
#         f.write("%s\n" % item)
