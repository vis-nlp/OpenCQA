...
def sample_sequence(model, length, context, transfer_learn, load_format, num_samples=1, temperature=1, stop_token=None, \
        ...
        for _ in range(length):
            if load_format == 'krl':
                if transfer_learn == True:
                    outputs = model(inputs_embeds = context)
                else:
                    outputs = model(inputs_embeds = context)[0]
            else:
                if transfer_learn == True:
                    outputs = model(inputs_embeds = context)
                else:
                    outputs = model(context)[0]
