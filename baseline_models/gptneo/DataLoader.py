...
class Dataloader(object):
    def __init__(self, train_name, val_name, test_name):
        if train_name:
            with open(train_name, 'r', encoding = 'utf8') as f:
                self.train = json.load(f)
            # self.train_ids = list(self.train.keys())

        if val_name:
            with open(val_name, 'r', encoding = 'utf8') as f:
                self.val = json.load(f)
            # self.val_ids = list(self.val.keys())

        if test_name:
            with open(test_name, 'r', encoding = 'utf8') as f:
    ...
    def get_reference(self, idx, option='test'):
        assert option == 'test'
        entry = self.test[self.test_index(idx)][4]
    ...
    def get_table_id(self, idx, option='test'):
        assert option == 'test'
        # table_id = self.test_ids[idx]
        return self.test_index(idx)

    def shuffle(self):
        random.shuffle(self.train_ids)

    def train_index(self,index):
        return list(self.train.keys())[index]

    def val_index(self,index):
        return list(self.val.keys())[index]

    def test_index(self,index):
        return list(self.test.keys())[index]
    
    def extract_krl(self,shape):
...
class GPTNeoLoadData(Dataloader):
    def __init__(self, train_name, val_name, test_name, tokenizer, load_format, bbox_path, batch_size=5):
        super(GPTNeoLoadData, self).__init__(train_name, val_name, test_name)
        # if train_name:
        #     with open(train_name, 'r', encoding = 'utf8') as f:
        #         self.train = json.load(f)
    ...
    def load_charts(self,load_format,e):
    ...
    def get_train_data(self, idx):
        ...
        # start_idx = max(0, idx - self.batch_size)
        start_idx = idx
        end_idx = min(idx + self.batch_size, len(self.train))
        entries = []
        for sample in range(start_idx,end_idx):
            entries.append(self.train[self.train_index(sample)])
...
#input format: chart_embeddings+question+title
...
# make all rows same length for tensor creation
        ...
        seqs = torch.LongTensor(seqs) #output tensor
        seq_masks = torch.FloatTensor(seq_masks)

        length = max([len(_) for _ in encoder_input]) + 1
        for i in range(len(encoder_input)):
            encoder_input[i] = (length - len(encoder_input[i])) * [self.tokenizer.eos_token_id] + encoder_input[i]
        encoder_input = torch.LongTensor(encoder_input) #input tensor

        decoder_inputs = seqs[:, :-1]
        decoder_outputs = seqs

        return decoder_inputs, decoder_outputs, seq_masks, encoder_input
    ...
    def get_test_data(self, idx):
        ...
        entries = []
        # for sample in range(start_idx,end_idx):
        entries.append(self.test[self.test_index(idx)])
...
#input format: question+title+chart_embeddings
#output format: answer
        for e in entries:
