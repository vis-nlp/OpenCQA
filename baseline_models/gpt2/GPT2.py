...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_from', default='checkpoints/GPT2_ep4.pt', type=str, help="load model path")
    ...
    parser.add_argument('--max_len', default=512, type=int, help="max seq length model can take is 2048")
    ...
    if args.do_train:
        ...
        tb_writer = SummaryWriter(log_dir='tensorboard/GPT2_ep/{}'.format(recording_time))        
        dataset = GPT2LoadData('data/train.json', None, None,tokenizer, args.chart_load_format, args.load_bboxes_from, args.batch_size)
        # if args.stage == 2:
...
#start transfer learning             
                    ...
                    torch.save(model.state_dict(), '{}/GPT2_ep_-1.pt'.format(args.id))
#start fine tuning
        ...
        for epoch_idx in range(args.epoch):
            ...
            for idx in range(0, dataset.train_len()):
                ...
                # decoder_inputs = decoder_inputs[:,:args.max_len]
                inputs = torch.cat([encoder_input, decoder_inputs], 1)
                inputs = inputs[:,:args.max_len]
            ...
            #if args.model == 'gpt2':
            torch.save(model.state_dict(), '{}/GPT2_ep{}.pt'.format(args.id, epoch_idx))
            # else:
    ...
    if args.do_test:
        ...
        dataset = GPT2LoadData(None, None, 'data/test.json',tokenizer, args.chart_load_format, args.load_bboxes_from, args.batch_size)
        if args.transfer_learn == True:
